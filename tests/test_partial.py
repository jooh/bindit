...
def test_stdout_bash(shebang="#!/bin/bash", vararg_pattern='"$@"'):
    ...
    with test_docker.tempfile.TemporaryDirectory(prefix=test_docker.TEMPFILE_PREFIX) as sourcedir:
        ...
        args = ("bindit_partial", "--shebang", shebang,
                "--vararg_pattern", vararg_pattern, "bindit", "docker", "run",
                *manual_bind,
                *remaining_arg)
        result = bindit.shell.run(*args, interactive=False)
        lines = result.stdout.split("\n")
        partialtester(lines, shebang, vararg_pattern,
                " ".join(manual_bind), " ".join(remaining_arg))
...
def test_file_bash(shebang="#!/bin/bash", vararg_pattern='"$@"'):
    ...
    with test_docker.tempfile.TemporaryDirectory(prefix=test_docker.TEMPFILE_PREFIX) as sourcedir:
        ...
        remaining_arg = (test_docker.IMAGE, "bash", "-x")
        args = ("bindit_partial", "--shebang", shebang,
                "--vararg_pattern", vararg_pattern, "--output_file", output_file, "bindit", "docker", "run",
                *manual_bind,
                *remaining_arg)
        result = bindit.shell.run(*args, interactive=False)
...
def test_docker_direct():
    ...
    with test_docker.tempfile.TemporaryDirectory(prefix=test_docker.TEMPFILE_PREFIX) as sourcedir:
        ...
        manual_bind = bindit.docker.volume_bind_args(sourcedir_resolved, manual_dest)
        remaining_arg = (test_docker.IMAGE, "bash", "-x")
        args = ("bindit_partial", "docker", "run",
                *manual_bind,
                *remaining_arg)
        result = bindit.shell.run(*args, interactive=False)
        lines = result.stdout.split("\n")
        partialtester(lines, "#!/bin/bash", '"$@"',
                " ".join(manual_bind), " ".join(remaining_arg))
